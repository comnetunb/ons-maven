...
/**
 * The Space Division Multiplexing Elastic Optical Network (SDM-EON) ons.Core represents a Fiberlink core in an optical
...
public class Core {
    ...
    public boolean leftExtend(int requiredSlots, long lp) {
        ...
        int count = 0;
        if (position[0] - 1 > -1 && slots[position[0] - 1] == -1) {
            for (int i = position[0] - 1; i > -1 && i > (position[0] - 1 - requiredSlots - guardband); i--) {
    ...
    public boolean rightExtend(int requiredSlots, long lp) {
        ...
        int count = 0;
        if (position[1] + 1 < numSlots && slots[position[1] + 1] == -1) {
            for (int i = position[1] + 1; i < numSlots && i < (position[1] + 1 + requiredSlots + guardband); i++) {
                if (slots[i] == -1 || slots[i] == 0) {
                    count++;
                }
            }
