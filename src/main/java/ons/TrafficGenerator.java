...
public class TrafficGenerator {
    ...
    /**
     * Creates a new ons.TrafficGenerator object. Extracts the traffic information
     ...
     * @param pt the ons.PhysicalTopology object
     */
    public TrafficGenerator(Element xml, double forcedLoad, PhysicalTopology pt) {
        int rate, cos, weight, ids, idd, sources, dataAmount, trafficType, delta, DCid;
        ...
        for (int i = 0; i < numberCallsTypes; i++) {
            ...
            switch(trafficType) {
                ...
                case 1:
                    ...
                    if (Simulator.verbose) {
                        System.out.println("#################################");
                        System.out.println("Type of traffic: ons.BulkData");
                ...
                case 2:
                    ...
                    if (Simulator.verbose) {
                        System.out.println("#################################");
                        System.out.println("Type of traffic: ons.Batch");
        ...
        if (pairlist.getLength() > 0) {
            ...
            if (Simulator.verbose) {
                System.out.println("Asymmetric traffic enabled\nNumber of pairs reported: " + Integer.toString(numberTrafficPairTypes));
            }
            pairTypesInfo = new TrafficPairsInfo[numberTrafficPairTypes];
            TotalPairsWeight = numberTrafficPairTypes;
            for (int i = 0; i < pairlist.getLength(); i++) {
                TotalPairsWeight += Integer.parseInt(((Element) pairlist.item(i)).getAttribute("weight")) - 1;
            }
            int index = 0;
            for (int i = 0; i < pt.getNumNodes(); i++) {
                for (int j = 0; j < pt.getNumNodes(); j++) {
                    if (i != j) {
                        pairTypesInfo[index++] = new TrafficPairsInfo(i, j, 1);
                    }
                }
            }
            for (int i = 0; i < pairlist.getLength(); i++) {
                ...
                if (Simulator.verbose) {
                    System.out.println("#################################");
                    System.out.println("Id Source: " + Integer.toString(ids) + ".");
                    System.out.println("Id Destination: " + Integer.toString(idd) + ".");
                    System.out.println("Weight: " + Integer.toString(weight) + ".");
                }
            }
            if (Simulator.verbose) {
     ...
     *
     * @param events ons.EventScheduler object that will contain the simulation
     * events
