...
//            }
     ...
     * 
     * @param pred list of the destination node's predecessors
     * @param src the source node
     * @param dst the destination node
    ...
    public static void printPath(int[] pred, int src, int dst) {
        ...
        int x = dst;
        while (x != src) {
            path.add(0, x);//path.add(0, G.getLabel(x));
            x = pred[x];
        }
        path.add(0, src);//path.add(0, G.getLabel(s));
...
//            }
            if (next >= 0) {
                ...
                for (int j = 0; j < n.length; j++) {
                    final int v = n[j];
                    final double d = dist[next] + G.getWeight(next, v, index);
                    if (dist[v] > d) {
                        dist[v] = d;
                        pred[v] = next;
                    }
     ...
     * @param src the source node
     * @param dst the destination node
     * @param index the index of multiWeight edge
     * @return the shortest path, as a vector of integers that represent node coordinates
     */
    public static int[] getShortestPath(WeightedMultiGraphMultiWeight G, int src, int dst, int index) {
        ...
        final int[] pred = dijkstra(G, src, index);
...
//            }
            if (next >= 0) {
                ...
                final int[] n = G.neighbors(next, index);
                for (int j = 0; j < n.length; j++) {
                    final int v = n[j];
                    final double d = dist[next] + G.getWeight(next, v, index);
                    if (dist[v] > d) {
