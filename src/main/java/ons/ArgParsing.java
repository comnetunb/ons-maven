...
public final class ArgParsing {
    ...
    private static final String usage = "Usage: EONSim -f <simulation_file> -s <seed> [-trace] [-ons.ra <raClass>] [ [-l <load>] or [-L <minload maxload step>] ] [-verbose | -table | -json]";
    ...
    private static void printHelp() {
        ...
        System.out.println("Arguments:\n"
                ...
                + "\t-ons.ra <raClass>                   Routing Algorithm Class\n"
    ...
    static void parse(String[] args) {
        ...
        int flags = 0, loads = 0, outputs = 0, seeds = 0, files = 0, ras = 0, traces = 0;
        ...
        flags += loads + outputs + seeds + files + ras + traces;
        if (flags < 2 || flags > 6) {
            printUsage();
        } else {
            if (loads > 1) {
            ...
            if (ras > 1) {
                System.out.println("Were declared two flags '-ons.ra'");
                printUsage();
            }
            if (traces > 1) {
